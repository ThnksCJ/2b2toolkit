...
public class DoubleSlider extends Component {
    ...
    public DoubleSlider(DoubleSetting doubleSetting, Button button, int n) {
    ...
    public void updateComponent(int n, int n2) {
        ...
        if (this.dragging) {
            if (d == 0.0) {
                this.val.setValue(this.val.getMinimumValue());
            } else {
                double d4 = DoubleSlider.roundToPlace(d / this.parentWidth * (d3 - d2) + d2, 2);
    ...
    public void renderComponent() {
        ...
        GUIUtils.drawRectangle(this.parent.parent.getX() + 6, this.parent.parent.getY() + this.offset + 15, (int) (smoothWidth < 0 ? 0 : smoothWidth), 4, Gui.color);
