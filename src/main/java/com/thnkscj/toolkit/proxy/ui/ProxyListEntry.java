...
public class ProxyListEntry implements GuiListExtended.IGuiListEntry {
    ...
    public ProxyListEntry(ProxyListGui ownerIn, ServerData serverIn) {
        this.owner = ownerIn;
        this.server = serverIn;
        this.mc = Minecraft.getMinecraft();
        this.serverIcon = new ResourceLocation("servers/2b2toolkit/proxy/" + serverIn.serverIP + "/icon");
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
        if (!this.server.pinged) {
            ...
            EXECUTOR.submit(() -> {
                try {
                    ProxyListEntry.this.oldServerPinger.ping(ProxyListEntry.this.server);
                } catch (UnknownHostException var2) {
                    ProxyListEntry.this.server.pingToServer = -1L;
                    ProxyListEntry.this.server.serverMOTD = TextFormatting.DARK_RED + I18n.format("multiplayer.status.cannot_resolve");
                } catch (Exception var3) {
                    ProxyListEntry.this.server.pingToServer = -1L;
                    ProxyListEntry.this.server.serverMOTD = TextFormatting.DARK_RED + I18n.format("multiplayer.status.cannot_connect");
                }
            });
        }
